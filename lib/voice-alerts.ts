...
export type VoiceAlertEvent = "red_signal_ahead" | "green_signal_changed" | "lane_departure" | "speed_violation";
...
export function buildVoiceAlertText(
  ...
  if (settings.length === "detailed") {
    ...
    if (event === "lane_departure") {
      return appendSupplementalText(`차선 이탈 감지.${STYLE_SUFFIX[settings.style]}`);
    }

    if (event === "speed_violation") {
      return appendSupplementalText(supplementalText || `속도 위반 감지.${STYLE_SUFFIX[settings.style]}`);
    }
  ...
  if (event === "lane_departure") {
    return appendSupplementalText(supplementalText || "차선 이탈.");
  }

  if (event === "speed_violation") {
    return appendSupplementalText(supplementalText || "속도 위반.");
  }
