...
describe("camera and settings controls regression", () => {
  ...
  it("keeps camera range controls and real signal recognition wiring", () => {
    ...
    expect(cameraSource).toContain('AUTO_SCAN_INTERVAL_MS = 2200');
    ...
    expect(cameraSource).toContain('setLiveStatusText("실시간 스캔 준비 중")');
    expect(cameraSource).toContain('연속 스캔 켜짐');
    expect(cameraSource).toContain('연속 스캔 꺼짐');
    expect(cameraSource).toContain('Text style={[styles.secondaryButtonText, isMonitoring && styles.secondaryButtonTextActive]}>');
